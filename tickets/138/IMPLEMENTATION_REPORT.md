...
Не выполнено в этой сессии (нет доступа к БД/браузеру/устройству):
- `doc/db/date_scoped_daily_tables.sql` не применён к БД.
- Ручная проверка: реальный скан (единственный путь после тикета 137)
  ...
  ошибок `ON CONFLICT`.
- Ручная проверка: чтение текущих RLS-политик через Supabase SQL Editor
  подтверждает отсутствие широких `USING (true)`-политик на обеих таблицах.
