...
| [122](122/122.txt) | Серверная логика обработки пакетов сканирования (приём/хранение `scan_packets`, идемпотентность, батч-триггеры, UI администратора) — реализуется одновременно с 120, на основе research-а 121. `122.txt` переработан в самостоятельное ТЗ (не дублирует 120/121). План: [tickets/122/IMPLEMENTATION_PLAN.md](122/IMPLEMENTATION_PLAN.md) — таблица `scan_packets`, батч-триггеры (transition tables) на `on_scan_insert`/`on_children_today_change`, Edge Function `submit-scan-packet` + функция БД `submit_scan_packet()`, композабл `useScanPackets.js`, новый `GroupDetailModal.vue` + доработка `AdminBusView.vue`/`BusDetailModal.vue`/`ChildrenView.vue` | ARCHITECT_DONE |
