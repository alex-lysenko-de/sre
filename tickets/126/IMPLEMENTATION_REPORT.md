...
# Удаление Kopfzählung (тикет 123)

По прямому запросу пользователя функциональность Kopfzählung (123)
признана неудачным экспериментом и удалена целиком:

- Удалены `src/views/HeadcountView.vue`, `src/composables/useGroupScanSession.js`.
- Убран маршрут `/headcount` из `src/router/index.js`.
- Убраны кнопки "Kopfzählung"/"Headcount" и обработчики `goToHeadcount()`
  из `src/views/MainView.vue` и `src/views/GroupEditView.vue` (включая
  ставший ненужным `canShowHeadcountButton` в `GroupEditView.vue`).
- `src/components/scanner/Scanner.vue` очищен от всего, что было
  добавлено в 126 исключительно ради встроенной панели Kopfzählung:
  проп `autoStart`, состояние "камера выключена" с кнопкой "Scan" внутри
  области сканера, emits `camera-started`/`camera-stopped`, методы
  `start()`/`stop()` в `defineExpose` (оставлен только `showMessage`,
  который активно используют `ScannerBusView`/`ScannerGroupView`/
  `ScannerCheckinView`). Камера снова всегда стартует в `onMounted`, без
  условия — как до 126. Bus/Group/Checkin режимы поведенчески не
  затронуты (они всегда использовали `autoStart` по умолчанию `true` и
  никогда не вызывали `start()`/`stop()` извне).

`npm run build` проходит без ошибок после обоих наборов правок.
