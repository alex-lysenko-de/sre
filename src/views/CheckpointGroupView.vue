...
<!-- Ticket 131 - reale Detail-/Monitoring-Ansicht fuer eine Group-Checkpoint.
     Ported from the 130_2 prototype (CheckpointGroupPrototypeView.vue) -
     Datenquelle wechselt auf useCheckpoints.js (echte Supabase-Abfragen),
     DebugTag entfernt.

     Anpassung fuer echte Daten: getGroupChildrenBreakdown() ist jetzt async
     (echte DB-Abfrage) - wird daher in load() vorab berechnet
     (breakdownCounts als ref) statt als sync computed(). -->
<template>
...
async function load() {
  ...
  if (checkpoint.value) {
    ...
    const breakdown = await getGroupChildrenBreakdown(checkpoint.value)
