  ...
  <div class="main-container">
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div v-if="loadingInitialData || configStore.loading" class="text-center py-5">
          <div class="spinner-border mb-3" role="status">
            <span class="visually-hidden">Wird geladen...</span>
          </div>
          <p class="text-muted">Lade {{ configStore.loading ? 'Konfiguration' : 'Tagesdaten' }}...</p>
        </div>

        <div v-else-if="configStore.error" class="text-center py-5">
          <font-awesome-icon :icon="['fas', 'exclamation-triangle']" />
          <h5 class="text-muted">Konfiguration konnte nicht geladen werden.</h5>
          <p class="text-muted">Fehler: {{ configStore.error.message }}. Standardwerte werden verwendet.</p>
        </div>

        <div v-else>
          <ul class="list-group list-group-flush children-list">
            <li v-if="days.length === 0" class="list-group-item text-center text-muted">
              Keine Tage gespeichert.
            </li>
            ...
            <li v-for="day in days" :key="day.id" class="list-group-item d-flex justify-content-between align-items-center">
              <div>
                ...
                <p v-if="day.description" class="text-muted mb-0 mt-1" style="font-size: 0.9em;">
                  Beschreibung: {{ day.description.substring(0, 50) + (day.description.length > 50 ? '...' : '') }}
                </p>
              ...
              <div class="d-flex">
                <button class="btn btn-outline-primary btn-sm me-2" @click="editDay(day.id)" title="Tag bearbeiten">
                  <font-awesome-icon :icon="['fas', 'edit']" />
                </button>
                <button class="btn btn-outline-danger btn-sm" @click="removeDay(day.id, day.date)" title="Tag löschen">
    ...
    <div class="card mt-4">
      <div class="card-header">
        <h3 class="mb-0">
          ...
          {{ editingDayId !== null ? 'Tag bearbeiten:' : 'Tag hinzufügen / bearbeiten' }}
        </h3>
      </div>
      <div class="card-body">
        ...
        <form @submit.prevent="saveDay">
          ...
          <div class="mb-3">
            <label for="newDayName" class="form-label">Reiseziel</label>
            <input
                type="text"
                id="newDayName"
                class="form-control"
          ...
          <div class="row">
            <div class="col-md-6 mb-3">
              <label for="newDayAbfahrt" class="form-label">Abfahrtszeit <span class="text-danger">*</span></label>
              <input
                  type="time"
                  id="newDayAbfahrt"
                  class="form-control"
            ...
            <div class="col-md-6 mb-3">
              <label for="newDayAnkommen" class="form-label">Ankunftszeit  <span class="text-danger">*</span></label>
              <input
                  type="time"
                  id="newDayAnkommen"
                  class="form-control"
          ...
          <div class="mb-4">
            <label for="newDayDescription" class="form-label">Beschreibung</label>
            <textarea
                id="newDayDescription"
                class="form-control"
                v-model="newDayDescription"
                rows="3"
                placeholder="Details zu den Aktivitäten des Tages"
            ></textarea>
          </div>

          <div class="d-flex justify-content-between">
            <button type="submit" class="btn btn-primary btn-lg flex-grow-1 me-2" :disabled="configStore.loading">
              <font-awesome-icon :icon="['fas', editingDayId !== null ? 'save' : 'plus']" />
              {{ editingDayId !== null ? 'Speichern' : 'Hinzufügen' }}
            </button>
...
export default {
  ...
  data() {
    return {
      ...
      newDayName: '',
      newDayAbfahrt: '', // Default from config
      newDayAnkommen: '', // Default from config
      newDayDescription: '',
  ...
  methods: {
    ...
    /**
     * Sets default abfahrt and ankommen values from the Pinia config store.
     */
    setDefaultsFromConfig() {
      // Access config directly from the store
      const abfahrtTime = this.configStore.config.abfahrtszeit
      const ankommenTime = this.configStore.config.ankunftszeit
      ...
      this.isConfigLoaded = !this.configStore.error

      if (this.configStore.error) {
        this.showAlert(`Fehler beim Laden der Konfiguration: ${this.configStore.error.message}. Standardwerte (08:00/17:00) wurden verwendet.`, 'warning')
      } else if (!abfahrtTime || !ankommenTime) {
        // Show warning if config does not contain required keys, but loading error did not occur
        this.showAlert(`Die Schlüssel 'abfahrtszeit' oder 'ankunftszeit' wurden in der Supabase-Konfiguration nicht gefunden. Standardwerte (08:00/17:00) wurden verwendet.`, 'warning')
      }
    },
    ...
    async loadInitialData() {
      ...
      try {
        const data = await this.fetchDaysList()
        this.days = data
        this.showAlert(`Tagesdaten wurden von Supabase geladen.`, 'info')
      } catch (error) {
        this.showAlert(`Fehler beim Laden der Tagesdaten: ${error.message}`, 'danger')
      } finally {
    ...
    validateDayData() {
      ...
      if (!date) {
        this.showAlert('Bitte wählen Sie ein Datum aus.', 'warning', 'formAlertContainer')
        return false
      }
      if (!abfahrt || !ankommen) {
        this.showAlert('Bitte geben Sie die Abfahrts- und Ankunftszeit an.', 'warning', 'formAlertContainer')
        return false
      }
    ...
    editDay(dayId) {
      ...
      if (dayToEdit) {
        ...
        this.showAlert(`Der Tag "${dayToEdit.date}" wurde zum Bearbeiten geladen.`, 'info', 'formAlertContainer')
      }
    },

    async removeDay(dayId, dayDate) {
      if (confirm(`Möchten Sie den Tag "${dayDate}" wirklich löschen?`)) {
        try {
          ...
          this.showAlert(`Der Tag "${dayDate}" wurde erfolgreich gelöscht.`, 'success')
        } catch (error) {
          this.showAlert(`Fehler beim Löschen des Tages: ${error.message}`, 'danger')
        }
      }
    },

    goBack() {
      alert('Zurück-Navigation (Emulation): Zurück zur vorherigen Ansicht.')
    }
  }
}
</script>
