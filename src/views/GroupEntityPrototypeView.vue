<!-- src/views/GroupEntityPrototypeView.vue -->
<!-- Ticket 130_2 - UX-Feedback Runde 4, Punkt 8: neue, Checkpoint-
     uebergreifende Entitaet "Gruppe" - Gruppennummer, Kinder-/Betreuer-
     Liste (derselbe EntityListCard-Baustein wie ueberall sonst), aktuelles
     Ergebnis (Link zur zugehoerigen, noch Checkpoint-gebundenen
     Gruppenansicht, Page 3) und die Tageshistorie aller GROUP-Checkpoints
     dieser Gruppe. Zu unterscheiden von der bestehenden Checkpoint-
     gebundenen Ansicht (/admin/checkpoints-prototype/group/:id) - diese
     hier ist tagesuebergreifend, siehe useGroupEntityMock.js. -->
<template>
  <div class="cp-group-entity-view">
    <DebugTag variant="page" label="Page 9" />
...
import { fetchGroupEntity } from '@/composables/useGroupEntityMock'
...
function goToCheckpointGroup(checkpointId) {
  router.push(`/admin/checkpoints-prototype/group/${checkpointId}`)
}
