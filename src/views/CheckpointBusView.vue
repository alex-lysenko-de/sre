...
<!-- Ticket 131 - reale Detail-/Monitoring-Ansicht fuer eine Bus-Checkpoint.
     Ported from the 130_2 prototype (CheckpointBusPrototypeView.vue) - alle
     4 UX-Feedback-Runden bleiben erhalten, nur die Datenquelle wechselt von
     useCheckpointsMock.js zu useCheckpoints.js (echte Supabase-Abfragen).
     DebugTag entfernt (war nur fuer die UX-Abstimmung von 130_2).

     Anpassung fuer echte Daten: getBusDelta()/getBusChildrenBreakdown()
     sind jetzt async (echte DB-Abfragen) statt synchroner Mock-Berechnung -
     werden daher in load() vorab berechnet (busDeltas/breakdownCounts) statt
     inline im Template aufgerufen. -->
<template>
...
async function load() {
  ...
  if (checkpoint.value) {
    ...
    const breakdown = await getBusChildrenBreakdown(checkpoint.value)
    ...
    const deltaEntries = await Promise.all(
        checkpoint.value.buses.map(async bus => [bus.busNumber, await getBusDelta(checkpoint.value, bus.busNumber)])
    )
    busDeltas.value = Object.fromEntries(deltaEntries)
  } else {
    resultSummary.value = null
    breakdownCounts.value = { present: 0, absent: 0, betreuer: 0 }
    busDeltas.value = {}
  }
