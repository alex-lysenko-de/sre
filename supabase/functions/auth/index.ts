...
async function handleRegisterPrepare(req: Request) {
  try {
    ...
    if (!inviteToken || !displayName) {
      return new Response(
        JSON.stringify({ error: 'Missing inviteToken or displayName' }),
    ...
    const { error: challengeError } = await supabase
      ...
      .insert({
        ...
        type: 'registration',
        user_data: { displayName, inviteToken },
...
// ===== REGISTER FINISH =====
async function handleRegisterFinish(req: Request) {
  try {
...
// ===== LOGIN PREPARE =====
async function handleLoginPrepare(req: Request) {
  try {
...
// ===== LOGIN FINISH =====
async function handleLoginFinish(req: Request) {
  try {
